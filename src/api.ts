    ...
    const server = http.createServer(async (req, res) => {
        try {
            ...
            if (req.method === 'POST' && req.url === '/watch') {
                ...
                await pool.query(
                    `INSERT INTO watched_addresses (address_base58,address_hex40,active,expires_at,label,added_by)
           ...
           ON DUPLICATE KEY UPDATE active=1, address_hex40=VALUES(address_hex40),
             ...
             label=IFNULL(:label,label), added_by=IFNULL(:by,added_by)`,
                    { b: dto.address, h: hex40, hrs: dto.expiresInHours, label: dto.label ?? null, by: dto.addedBy ?? null }
                );
